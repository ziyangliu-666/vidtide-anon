"""NSG-VD Crafter detector — MMD-MP trained by us on GenVideo Crafter/MSR-VTT.

Mirrors nsgvd_pika.py byte-for-byte except for:
  - ckpt path: Checkpoints/nsgvd_crafter.pth (see scripts/modal/nsgvd_crafter_train.py)
  - ref cache namespace: ref_crafter_*
  - registered as "nsgvd_crafter"

The official NSG-VD release ships only Pika/SEINE ckpts; this module fills the
Crafter slot in tab:gap / tab:pergen / tab:perplat of the paper.
"""
...
_DEFAULT_CKPT = _REPO / "Checkpoints" / "nsgvd_crafter.pth"
_DIFFUSION_CKPT = _REPO / "Checkpoints" / "256x256_diffusion_uncond.pt"
_REF_CACHE_DIR = _REPO / ".nsgvd_ref_cache"
_DIFFUSE_STEPS = 5


@register_detector("nsgvd_crafter")
class NSGVDCrafter(BaseDetector):
    name = "nsgvd_crafter"
    expects_frames = 8
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not ckpt.exists():
            raise FileNotFoundError(
                f"NSG-VD Crafter ckpt not found: {ckpt}\n"
                "Train via: modal run scripts/modal/nsgvd_crafter_train.py"
            )
        ...
        try:
            ...
            logger.info("NSGVDCrafter: loading %s on %s", ckpt, self.device)
            ...
            logger.info("NSGVDCrafter: loading diffusion score_fn (%s)", _DIFFUSION_CKPT.name)
    ...
    def _extract_velocity(self, frames: np.ndarray) -> torch.Tensor:
        ...
        with torch.no_grad():
            score_full = self.score_fn(2 * flat - 1, curr_t)
    ...
    def _build_ref_features(self, ref_videos: list[str]) -> None:
        ...
        cache_file = _REF_CACHE_DIR / f"ref_crafter_{cache_key}_n{len(ref_videos)}.pt"
        ...
        if cache_file.exists():
            ...
            logger.info(
                "NSGVDCrafter: loaded cached ref features from %s (N=%d)",
                cache_file.name, self.feature_ref.shape[0],
            )
            return

        logger.info("NSGVDCrafter: extracting ref features for %d videos", len(ref_videos))
        velocities = []
        ...
        logger.info(
            "NSGVDCrafter: cached ref features to %s (N=%d)",
            cache_file.name, self.feature_ref.shape[0],
        )
    ...
    def predict(self, frames: np.ndarray) -> float:
        ...
        velocity = self._extract_velocity(frames)
        with torch.no_grad():
            _, feat = self.model.net(velocity, out_feature=True)
            n_ref = self.feature_ref.shape[0]
            Fea = torch.cat([self.feature_ref, feat], dim=0)
