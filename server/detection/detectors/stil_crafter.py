"""STIL (ACM MM'21, SCNet50 backbone) detector — VideoCrafter-trained from NSG-VD repo."""
...
_DEFAULT_CKPT = _NSGVD_DIR / "results" / "ckpts" / "baselines" / "standard-Crafter-stil" / "final_ckpt.pth"
_IMAGENET_MEAN = torch.tensor([0.485, 0.456, 0.406]).view(1, 1, 3, 1, 1)
_IMAGENET_STD = torch.tensor([0.229, 0.224, 0.225]).view(1, 1, 3, 1, 1)


@register_detector("stil_crafter")
class STILCrafter(BaseDetector):
    name = "stil_crafter"
    expects_frames = 8
    ...
    def __init__(self, device: str = "cuda", ckpt_path: str | None = None, **kwargs) -> None:
        ...
        logger.info("STILCrafter: loading %s on %s", ckpt, self.device)
