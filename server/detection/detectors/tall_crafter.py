"""TALL (ICCV'23, Swin-B backbone) detector — VideoCrafter-trained from NSG-VD repo."""
...
_DEFAULT_CKPT = _NSGVD_DIR / "results" / "ckpts" / "baselines" / "standard-Crafter-tall" / "final_ckpt.pth"
_IMAGENET_MEAN = torch.tensor([0.485, 0.456, 0.406]).view(1, 1, 3, 1, 1)
_IMAGENET_STD = torch.tensor([0.229, 0.224, 0.225]).view(1, 1, 3, 1, 1)


@register_detector("tall_crafter")
class TALLCrafter(BaseDetector):
    name = "tall_crafter"
    expects_frames = 8
    ...
    def __init__(self, device: str = "cuda", ckpt_path: str | None = None, **kwargs) -> None:
        ...
        if not ckpt.exists():
            raise FileNotFoundError(f"TALL checkpoint not found: {ckpt}")

        logger.info("TALLCrafter: loading %s on %s", ckpt, self.device)
        self.model = TALL_SWIN(pretrained=False).to(self.device)
    ...
    def predict(self, frames: np.ndarray) -> float:
        ...
        with torch.no_grad():
            logit = self.model(x)            # (1, 1) — TALL_SWIN default num_classes=1
