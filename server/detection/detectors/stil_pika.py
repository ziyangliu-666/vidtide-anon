"""STIL (ACM MM'21, SCNet50 backbone) detector — Pika-trained from NSG-VD repo."""
...
_DEFAULT_CKPT = _NSGVD_DIR / "results" / "ckpts" / "baselines" / "standard-Pika-stil" / "final_ckpt.pth"
_IMAGENET_MEAN = torch.tensor([0.485, 0.456, 0.406]).view(1, 1, 3, 1, 1)
_IMAGENET_STD = torch.tensor([0.229, 0.224, 0.225]).view(1, 1, 3, 1, 1)


@register_detector("stil_pika")
class STILPika(BaseDetector):
    name = "stil_pika"
    expects_frames = 8
    ...
    def __init__(self, device: str = "cuda", ckpt_path: str | None = None, **kwargs) -> None:
        ...
        # Patch SCNet50's pretrained loader to skip the missing ImageNet ckpt.
        import models.stil as stil_mod
        _orig_scnet = stil_mod.scnet50_v1d
        if not getattr(stil_mod, "_pretrained_disabled", False):
            stil_mod.scnet50_v1d = lambda ns, pretrained=False, **kw: _orig_scnet(ns, pretrained=False, **kw)
            stil_mod._pretrained_disabled = True
        from models.stil import Det_STIL

        self.device = device if torch.cuda.is_available() else "cpu"
        ckpt = Path(ckpt_path) if ckpt_path else _DEFAULT_CKPT
        if not ckpt.exists():
            raise FileNotFoundError(f"STIL checkpoint not found: {ckpt}")

        logger.info("STILPika: loading %s on %s", ckpt, self.device)
        self.model = Det_STIL().to(self.device)
    ...
    def predict(self, frames: np.ndarray) -> float:
        ...
        x = ((x - self._mean) / self._std).contiguous()
